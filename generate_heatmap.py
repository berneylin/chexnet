...
class HeatMapGenerator:
    def __init__(self):
        ...
        self.network_model = self.network_model.module.dense_net_121.features
        self.network_model.eval()  # set network as evaluation model
        self.network_model.cuda()

        # select the last convolution layers' weights as weights of CAM
        self.weights = list(self.network_model.parameters())[-2]
    ...
    def generator(self, path_to_raw_image, path_to_output_image, trans_crop=None):
        ...
        var_image = torch.autograd.Variable(image_data).cuda()

        with torch.autograd.no_grad():
            var_output = self.network_model(var_image)
